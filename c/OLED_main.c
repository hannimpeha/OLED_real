...
*/
	...
	for (i = 0; i < no_EML; i++)
	{
		for (j = 0; j < w_lgth; j++)
		{
			...
			for (k = 0; k < v_lgth; k++)
			{
				...
				L_2_TM[k][j] = comsqrt(commin(comprod(compow2(n_2[j]), inversecom(compow2(n_extra[j]))), comp(pow(inpv[k], 2))));
				L_2_TE[k][j] = comsqrt(commin(comprod(compow2(n_2[j]), inversecom(compow2(n_ordi[j]))), comp(pow(inpv[k], 2))));
				L_3_TM[k][j] = comsqrt(commin(comprod(compow2(n_3[j]), inversecom(compow2(n_extra[j]))), comp(pow(inpv[k], 2))));
				L_3_TE[k][j] = comsqrt(commin(comprod(compow2(n_3[j]), inversecom(compow2(n_ordi[j]))), comp(pow(inpv[k], 2))));
			...
			//	eq(2)
			P0_h[j] = comxreal(comprod(n_ordi[j], comprod(comsum(comxreal(compow2(n_ordi[j]), 3), compow2(n_extra[j])), inversecom(comxreal(compow2(n_ordi[j]), 4)))), Const);
			//	eq(3)
		...
		for (j = 0; j < w_lgth; j++)
		{
			for (k = 0; k < a_lgth; k++)
			{
				inpv_ext_TM[j][k] = comxreal(comprod(n_3[j], inversecom(n_extra[j])), sin(angle[k] * PI / (double)180));
				inpv_ext_TE[j][k] = comxreal(comprod(n_3[j], inversecom(n_ordi[j])), sin(angle[k] * PI / (double)180));

				inpv_sub_TM[j][k] = comxreal(comprod(n_2[j], inversecom(n_extra[j])), sin(angle[k] * PI / (double)180));
				...
				L_2_ext_TM[k][j] = comsqrt(commin(comprod(compow2(n_2[j]), inversecom(compow2(n_extra[j]))), compow2(inpv_ext_TM[j][k])));
				L_2_ext_TE[k][j] = comsqrt(commin(comprod(compow2(n_2[j]), inversecom(compow2(n_ordi[j]))), compow2(inpv_ext_TE[j][k])));

				L_2_sub_TM[k][j] = comsqrt(commin(comprod(compow2(n_2[j]), inversecom(compow2(n_extra[j]))), compow2(inpv_sub_TM[j][k])));
				L_2_sub_TE[k][j] = comsqrt(commin(comprod(compow2(n_2[j]), inversecom(compow2(n_ordi[j]))), compow2(inpv_sub_TE[j][k])));

				L_3_ext_TM[k][j] = comsqrt(commin(comprod(compow2(n_3[j]), inversecom(compow2(n_extra[j]))), compow2(inpv_ext_TM[j][k])));
				L_3_ext_TE[k][j] = comsqrt(commin(comprod(compow2(n_3[j]), inversecom(compow2(n_ordi[j]))), compow2(inpv_ext_TE[j][k])));
				...
				L_2_sub_ext_TM[k][j] = comsqrt(commin(comprod(compow2(n_3[j]), inversecom(compow2(n_2[j]))), compow2(inpv_sub_ext_TM[j][k])));
				L_2_sub_ext_TE[k][j] = comsqrt(commin(comprod(compow2(n_3[j]), inversecom(compow2(n_2[j]))), compow2(inpv_sub_ext_TE[j][k])));
			}
			*(rt_up_ext_TM + j ) = TMM_anisotropy_coeffs_compinpv_w(index_up, i, thick_up, *(inpv_ext_TM + j), a_lgth, no_up_layer[i], WL[j], j);
			*(rt_up_ext_TE + j ) = TMM_anisotropy_coeffs_compinpv_w(index_up, i, thick_up, *(inpv_ext_TE + j), a_lgth, no_up_layer[i], WL[j], j);
			*(rt_up_sub_TM + j ) = TMM_anisotropy_coeffs_compinpv_w(index_up, i, thick_up, *(inpv_sub_TM + j), a_lgth, no_up_layer[i], WL[j], j);
			*(rt_up_sub_TE + j ) = TMM_anisotropy_coeffs_compinpv_w(index_up, i, thick_up, *(inpv_sub_TE + j), a_lgth, no_up_layer[i], WL[j], j);

			*(rt_low_ext_TM + j ) = TMM_anisotropy_coeffs_compinpv_w(index_low, i, thick_low, *(inpv_ext_TM + j), a_lgth, no_low_layer[i], WL[j], j);
			*(rt_low_ext_TE + j ) = TMM_anisotropy_coeffs_compinpv_w(index_low, i, thick_low, *(inpv_ext_TE + j), a_lgth, no_low_layer[i], WL[j], j);
			*(rt_low_sub_TM + j ) = TMM_anisotropy_coeffs_compinpv_w(index_low, i, thick_low, *(inpv_sub_TM + j), a_lgth, no_low_layer[i], WL[j], j);
			*(rt_low_sub_TE + j ) = TMM_anisotropy_coeffs_compinpv_w(index_low, i, thick_low, *(inpv_sub_TE + j), a_lgth, no_low_layer[i], WL[j], j);

			*(rt_sub_ext_TM + j ) = TMM_anisotropy_coeffs_compinpv_w(index_sub_ext, i, thick_sub_ext, *(inpv_sub_ext_TM + j), a_lgth, no_of_layer_sub_ext, WL[j], j);
			*(rt_sub_ext_TE + j ) = TMM_anisotropy_coeffs_compinpv_w(index_sub_ext, i, thick_sub_ext, *(inpv_sub_ext_TE + j), a_lgth, no_of_layer_sub_ext, WL[j], j);
		}	//	wavelength�� loop
		...
		{	//	common process
			...
			for (k = 0; k < w_lgth; k++)
			{
				...
				{
					p_abs_TM_tailored[k][l] = p_total_TM[i][j][k][l] - p_total_out_12_TM[k][l] - p_total_out_13_TM[k][l];
					p_abs_TE_tailored[k][l] = p_total_TE[i][j][k][l] - p_total_out_12_TE[k][l] - p_total_out_13_TE[k][l];
				}

				//	integration in eq (7)
				P[i][j][k] = trapz(inpv, p_total[i][j][k], 1, v_lgth);
				OC[i][j][k] = trapz(inpv, p_total_out_12_TM[k], 1, ext_number_TM[k]) + trapz(inpv, p_total_out_12_TE[k], 1, ext_number_TE[k]);
				OC_back[i][j][k] = trapz(inpv, p_total_out_13_TM[k], 1, ext_number_TM[k]) + trapz(inpv, p_total_out_13_TE[k], 1, ext_number_TE[k]);
				ABS[i][j][k] = trapz(inpv, p_abs_TM_tailored[k], 1, ext_number_TM[k]) + trapz(inpv, p_abs_TE_tailored[k], 1, ext_number_TE[k]);
				SUBS[i][j][k] = trapz(inpv, p_total_TM[i][j][k], ext_number_TM[k], subs_number_TM[k]) + trapz(inpv, p_total_TE[i][j][k], ext_number_TE[k], subs_number_TE[k]);
				WG[i][j][k] = trapz(inpv, p_total_TM[i][j][k], subs_number_TM[k], WG_number_TM[k]) + trapz(inpv, p_total_TE[i][j][k], subs_number_TE[k], WG_number_TE[k]);
				SPPs[i][j][k] = trapz(inpv, p_total_TM[i][j][k], WG_number_TM[k], v_lgth) + trapz(inpv, p_total_TE[i][j][k], WG_number_TE[k], v_lgth);
			...
			for (k = 0; k < w_lgth; k++)
			{
				for (l = 0; l < a_lgth; l++)
				{
					Abs_R_v_TM_12_ext[l][k] = compabs2(comprod(commin(ONE, R_13_ext_TM[k][l]), inversecom(commin(ONE, R_1213_ext_TM[k][l])))).A;
					Abs_R_v_TM_13_ext[l][k] = compabs2(comprod(commin(ONE, R_12_ext_TM[k][l]), inversecom(commin(ONE, R_1213_ext_TM[k][l])))).A;
					Abs_R_v_TM_12_sub[l][k] = compabs2(comprod(commin(ONE, R_13_sub_TM[k][l]), inversecom(commin(ONE, R_1213_sub_TM[k][l])))).A;

					Abs_R_h_TM_12_ext[l][k] = compabs2(comprod(comsum(ONE, R_13_ext_TM[k][l]), inversecom(commin(ONE, R_1213_ext_TM[k][l])))).A;
					Abs_R_h_TM_13_ext[l][k] = compabs2(comprod(comsum(ONE, R_12_ext_TM[k][l]), inversecom(commin(ONE, R_1213_ext_TM[k][l])))).A;
					Abs_R_h_TM_12_sub[l][k] = compabs2(comprod(comsum(ONE, R_13_sub_TM[k][l]), inversecom(commin(ONE, R_1213_sub_TM[k][l])))).A;

					Abs_R_h_TE_12_ext[l][k] = compabs2(comprod(comsum(ONE, R_13_ext_TE[k][l]), inversecom(commin(ONE, R_1213_ext_TE[k][l])))).A;
					Abs_R_h_TE_13_ext[l][k] = compabs2(comprod(comsum(ONE, R_12_ext_TE[k][l]), inversecom(commin(ONE, R_1213_ext_TE[k][l])))).A;
					Abs_R_h_TE_12_sub[l][k] = compabs2(comprod(comsum(ONE, R_13_sub_TE[k][l]), inversecom(commin(ONE, R_1213_sub_TE[k][l])))).A;
				}
			}
			//	eq (25)
			multiply_5_1_1(prefactor_v_TM, Abs_R_v_TM_12_ext, T_12_ext_TM, inpv_ext_TM, L_1_ext_TM, n_2, L_2_ext_TM, n_extra, a_lgth, w_lgth, p_out_12_v_ext_TM);
			multiply_5_2(p_out_12_v_ext_TM, T_sub_ext_TM, n_3, L_2_sub_ext_TM, n_2, L_1_sub_ext_TM, a_lgth, w_lgth, p_out_12_v_ext_TM_intf);
			multiply_5_1_1(prefactor_v_TM, Abs_R_v_TM_13_ext, T_13_ext_TM, inpv_ext_TM, L_1_ext_TM, n_3, L_3_ext_TM, n_extra, a_lgth, w_lgth, p_out_13_v_ext_TM);
			multiply_5_1_1(prefactor_v_TM, Abs_R_v_TM_12_sub, T_12_sub_TM, inpv_sub_TM, L_1_sub_TM, n_2, L_2_sub_TM, n_extra, a_lgth, w_lgth, p_out_12_v_sub_TM);
			...
			multiply_5_2(p_out_12_h_ext_TE, T_sub_ext_TE, n_3, L_2_sub_ext_TE, n_2, L_1_sub_ext_TE, a_lgth, w_lgth, p_out_12_h_ext_TE_intf);
			multiply_5_1_3(prefactor_h_TE, Abs_R_h_TE_13_ext, T_13_ext_TE, L_1_ext_TE, n_3, L_3_ext_TE, n_extra, a_lgth, w_lgth, p_out_13_h_ext_TE);
			multiply_5_1_3(prefactor_h_TE, Abs_R_h_TE_12_sub, T_12_sub_TE, L_1_sub_TE, n_2, L_2_sub_TE, n_extra, a_lgth, w_lgth, p_out_12_h_sub_TE);

			multiply_4_1(p_out_12_v_ext_TM_intf, P0_v, EML[i].QY * (1 - EML[i].HDR), a_lgth, w_lgth, p_out_12_ext_TM_EMZ[i][j]);
			...
			multiply_4_1(p_out_12_h_ext_TE_intf, P0_v, EML[i].QY * EML[i].HDR, a_lgth, w_lgth, p_out_12_ext_TE_EMZ[i][j]);
			arrsum_new(p_out_12_ext_TM_EMZ[i][j], p_out_12_ext_TE_EMZ[i][j], w_lgth, a_lgth, p_out_12_ext_EMZ[i][j]);

			multiply_4_1(p_out_13_v_ext_TM, P0_v, EML[i].QY * (1 - EML[i].HDR), a_lgth, w_lgth, p_out_13_ext_TM_EMZ[i][j]);
			...
			multiply_4_1(p_out_12_v_sub_TM, P0_v, EML[i].QY * (1 - EML[i].HDR), a_lgth, w_lgth, p_out_12_sub_TM_EMZ[i][j]);
			...
			//	eq (27)
			multiply_4_2(p_out_12_ext_TM_EMZ[i][j], spectrum, i, EMZ[j][1][i], a_lgth, w_lgth, p_out_12_ext_TM_spec[i][j]);
			multiply_4_2(p_out_12_ext_TE_EMZ[i][j], spectrum, i, EMZ[j][1][i], a_lgth, w_lgth, p_out_12_ext_TE_spec[i][j]);
			arrsum_new(p_out_12_ext_TM_spec[i][j], p_out_12_ext_TE_spec[i][j], w_lgth, a_lgth, p_out_12_ext_spec[i][j]);

			multiply_4_2(p_out_13_ext_TM_EMZ[i][j], spectrum, i, EMZ[j][1][i], a_lgth, w_lgth, p_out_13_ext_TM_spec[i][j]);
			multiply_4_2(p_out_13_ext_TE_EMZ[i][j], spectrum, i, EMZ[j][1][i], a_lgth, w_lgth, p_out_13_ext_TE_spec[i][j]);
			arrsum_new(p_out_13_ext_TM_spec[i][j], p_out_13_ext_TE_spec[i][j], w_lgth, a_lgth, p_out_13_ext_spec[i][j]);

			multiply_4_2(p_out_12_sub_TM_EMZ[i][j], spectrum, i, EMZ[j][1][i], a_lgth, w_lgth, p_out_12_sub_TM_spec[i][j]);
			multiply_4_2(p_out_12_sub_TE_EMZ[i][j], spectrum, i, EMZ[j][1][i], a_lgth, w_lgth, p_out_12_sub_TE_spec[i][j]);
		...
		for (j = 0; j < w_lgth; j++)
		{
			OC_eff_integrated[i] += (spectrum[j][1][i] * EML[i].QY * OC_EML[j][i]) / (1 - EML[i].QY + EML[i].QY * P_EML[j][i]);
			OC_back_eff_integrated[i] += (spectrum[j][1][i] * EML[i].QY * OC_back_EML[j][i]) / (1 - EML[i].QY + EML[i].QY * P_EML[j][i]);
			ABS_eff_integrated[i] += (spectrum[j][1][i] * EML[i].QY * ABS_EML[j][i]) / (1 - EML[i].QY + EML[i].QY * P_EML[j][i]);
			SUBS_eff_integrated[i] += (spectrum[j][1][i] * EML[i].QY * SUBS_EML[j][i]) / (1 - EML[i].QY + EML[i].QY * P_EML[j][i]);
			WG_eff_integrated[i] += (spectrum[j][1][i] * EML[i].QY * WG_EML[j][i]) / (1 - EML[i].QY + EML[i].QY * P_EML[j][i]);
			SPPs_eff_integrated[i] += (spectrum[j][1][i] * EML[i].QY * SPPs_EML[j][i]) / (1 - EML[i].QY + EML[i].QY * P_EML[j][i]);
		}
	...
	free(ext_number_TM);	free(ext_number_TE);	free(subs_number_TM);	free(subs_number_TE);	free(WG_number_TM);	free(WG_number_TE);
	free2d(p_abs_TM_tailored); free2d(p_abs_TE_tailored);
	free2d(Temp);
	free(EML);	free(EXC_prop);
	free(n_ordi);	free(n_extra);	free(n_2);	free(n_3);
	free(P0_v);	free(P0_h);
	free2d(thick_up);	free2d(thick_low);	free2d(thick_sub_ext);
	free2c(L_1);	free2c(L_2_TE);	free2c(L_2_TM);	free2c(L_3_TE);	free2c(L_3_TM);
	free4c(rt_up);	free4c(rt_low);
	free4d(index_up);	free4d(index_low);
	free(no_up_layer);	free(no_low_layer);
	free3d(EMZ);
	free2c(r_12_TE);	free2c(t_12_TE);	free2c(r_13_TE);	free2c(t_13_TE);
	free2c(r_12_TM);	free2c(t_12_TM);	free2c(r_13_TM);	free2c(t_13_TM);
	free(prefactor_h_TE);	free(prefactor_h_TM);	free(prefactor_v_TM);
	free2d(Abs_R_v_TM_12);	free2d(Abs_R_h_TM_12);	free2d(Abs_R_h_TE_12);
	free2d(Abs_R_v_TM_13);	free2d(Abs_R_h_TM_13);	free2d(Abs_R_h_TE_13);
	free2c(T_12_TM);	free2c(T_13_TM);	free2c(T_12_TE);	free2c(T_13_TE);
	free2c(R_12_TM);	free2c(R_13_TM);	free2c(R_1213_TM);
	free2c(R_12_TE);	free2c(R_13_TE);	free2c(R_1213_TE);
	free2d(p_v_TM);	free2d(p_h_TM);	free2d(p_h_TE);
	free2d(p_out_12_v_TM); free2d(p_out_12_h_TM); free2d(p_out_12_h_TE);
	free2d(p_out_13_v_TM); free2d(p_out_13_h_TM); free2d(p_out_13_h_TE);
	free2d(p_total_out_12_TM); free2d(p_total_out_12_TE); free2d(p_total_out_12);
	free2d(p_total_out_13_TM); free2d(p_total_out_13_TE); free2d(p_total_out_13);
	free(inpv_cut_ext_TM);	free(inpv_cut_sub_TM);	free(inpv_cut_ext_TE);	free(inpv_cut_sub_TE);
	free2c(inpv_ext_TM);	free2c(inpv_ext_TE);	free2c(inpv_sub_TM); free2c(inpv_sub_TE);
	free2c(L_1_ext_TM); free2c(L_1_ext_TE); free2c(L_1_sub_TM); free2c(L_1_sub_TE);
	free2c(L_2_ext_TM); free2c(L_2_ext_TE); free2c(L_2_sub_TM); free2c(L_2_sub_TE);
	free2c(L_3_ext_TM); free2c(L_3_ext_TE);
	free2c(inpv_sub_ext_TM);	free2c(inpv_sub_ext_TE);
	free4d(index_sub_ext);
	free2c(L_1_sub_ext_TM); free2c(L_1_sub_ext_TE); free2c(L_2_sub_ext_TM); free2c(L_2_sub_ext_TE);
	free3c(rt_up_ext_TM); free3c(rt_up_ext_TE); free3c(rt_up_sub_TM); free3c(rt_up_sub_TE);
	free3c(rt_low_ext_TM); free3c(rt_low_ext_TE); free3c(rt_low_sub_TM); free3c(rt_low_sub_TE);
	free3c(rt_sub_ext_TM); free3c(rt_sub_ext_TE);
	free2c(r_12_ext_TM); free2c(t_12_ext_TM); free2c(r_12_sub_TM); free2c(t_12_sub_TM);
	free2c(r_12_ext_TE); free2c(t_12_ext_TE); free2c(r_12_sub_TE); free2c(t_12_sub_TE);
	free2c(r_13_ext_TM); free2c(t_13_ext_TM); free2c(r_13_ext_TE); free2c(t_13_ext_TE);
	free2c(r_13_sub_TM); free2c(r_13_sub_TE);
	free2c(t_sub_ext_TM); free2c(t_sub_ext_TE);
	free2c(T_12_ext_TM); free2c(T_13_ext_TM); free2c(T_12_sub_TM); free2c(T_sub_ext_TM);
	free2c(T_12_ext_TE); free2c(T_13_ext_TE); free2c(T_12_sub_TE); free2c(T_sub_ext_TE);
	free2c(R_12_ext_TM); free2c(R_13_ext_TM); free2c(R_1213_ext_TM); free2c(R_13_sub_TM); free2c(R_1213_sub_TM);
	free2c(R_12_ext_TE); free2c(R_13_ext_TE); free2c(R_1213_ext_TE); free2c(R_13_sub_TE); free2c(R_1213_sub_TE);
	free2d(Abs_R_v_TM_12_ext); free2d(Abs_R_v_TM_13_ext); free2d(Abs_R_v_TM_12_sub);
	free2d(Abs_R_h_TM_12_ext); free2d(Abs_R_h_TM_13_ext); free2d(Abs_R_h_TM_12_sub);
	free2d(Abs_R_h_TE_12_ext); free2d(Abs_R_h_TE_13_ext); free2d(Abs_R_h_TE_12_sub);
	free2d(p_out_12_v_ext_TM); free2d(p_out_12_v_ext_TM_intf); free2d(p_out_13_v_ext_TM); free2d(p_out_12_v_sub_TM);
	free2d(p_out_12_h_ext_TE); free2d(p_out_12_h_ext_TE_intf); free2d(p_out_13_h_ext_TE); free2d(p_out_12_h_sub_TE);
	free2d(p_out_12_h_ext_TM); free2d(p_out_12_h_ext_TM_intf); free2d(p_out_13_h_ext_TM); free2d(p_out_12_h_sub_TM);
	free3d(OC); free3d(OC_back); free3d(ABS);
	free3d(SUBS); free3d(WG); free3d(SPPs);
...
/*	free2d(OC_EML_spec); free2d(OC_back_EML_spec); free2d(ABS_EML_spec);	����
	...
	free3d(P_EMZ); free3d(OC_EMZ); free3d(OC_back_EMZ); free3d(ABS_EMZ);
	free3d(SUBS_EMZ); free3d(WG_EMZ); free3d(SPPs_EMZ);	free3d(P);	free(P0);
	free3d(spectrum);
	free4d(p_out_12_ext_TM_EMZ); free4d(p_out_12_ext_TE_EMZ); free4d(p_out_12_ext_EMZ);
	...
	double NR_loss_final = 1 - (OC_eff_final + OC_back_eff_final + ABS_eff_final + SUBS_eff_final + WG_eff_final + SPPs_eff_final);
	printf("OC:%g, OC_back:%g, Subs:%g, WG:%g, SPPs:%g,  Abs:%g, NR_loss:%g \n", OC_eff_final, OC_back_eff_final, SUBS_eff_final, WG_eff_final, SPPs_eff_final, ABS_eff_final, NR_loss_final);		//  NR_loss�� ABS��ġ �ٲ�Ͱ���	NR loss	�� 0??...
	//	mode analysis end
	free(OC_eff_integrated);	free(OC_back_eff_integrated);	free(ABS_eff_integrated);	free(SUBS_eff_integrated);
	...
	free3d(p_out_12_ext_TM_spec_EML); free3d(p_out_12_ext_TE_spec_EML); free3d(p_out_12_ext_spec_EML);
	free3d(p_out_13_ext_TM_spec_EML); free3d(p_out_13_ext_TE_spec_EML); free3d(p_out_13_ext_spec_EML);
...
//	char strFolderPathDS[100]={0};
	...
	free2d(p_out_12_ext_TE_final);	free2d(p_out_13_ext_TE_final); free2d(p_out_12_sub_TE_final);
	free3d(OC_eff);	free3d(OC_back_eff);	free3d(Purcell);	free3d(q_eff);
	free(output_angular_intensity_sub_TE);	free(output_angular_intensity_top_TE); free(output_angular_intensity_bottom_TE);
	free(output_angular_intensity_sub_TM); free(output_angular_intensity_top_TM); free(output_angular_intensity_bottom_TM);
	free(output_angular_intensity_sub); free(output_angular_intensity_top); free(output_angular_intensity_bottom);
